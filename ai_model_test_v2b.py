...
MODEL_PATH = '/home/admin02/testq/bulldog_sumtn_v15_256_e53_a5000.onnx'#VinUni-test
...
class SteeringPredictor:
    ...
    def routed_map_callback(self, msg):
        ...
        def save_routed_map(self):
            if self.routed_map is not None:
                try:
                    cv2.imwrite(self.routed_map_save_path, self.routed_map)
                    rospy.loginfo(f"Routed map saved at {self.routed_map_save_path}")
                except Exception as e:
                    rospy.logerr(f"Failed to save routed map: {e}") 
