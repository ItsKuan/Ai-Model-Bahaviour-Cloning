...
MIN_LIN_VEL={'1.1': 1.0, '1.2': 0.2, '1.3': 0.2,
             '1.4': 0.2, '1.5': 0.2, '1.6': 0.1, '1.7': 0.1,
...
def synchronize(main_folder_path, save_dir, rate, data_type='umtn', scenario='mix'):
    ...
    if data_type == 'umtn':
        ...
        timestamps = {key: [] for key in ['camera_front', 'gps', 'speed', 'joystick_speed', 'global_path', 'high_cmd', 'imu', 'routed_map']}
    elif data_type == 'bc':
    ...
    if data_type == 'umtn':
        ...
        df_csv = {key: read_and_clean_data(key, main_folder_path, timestamps) for key in ['gps', 'speed', 'joystick_speed', 'global_path', 'high_cmd', 'imu',]}
    elif data_type == 'bc':
    ...
    if data_type == 'umtn':
        ...
        final_df = pd.DataFrame(columns=['time', 'front_camera', 'routed_map',
                                         'angular_velocity(rad/s)', 'linear_velocity(m/s)',
                                         'joystick_angular_velocity(rad/s)', 'joystick_linear_velocity(m/s)',
                                        # 'ai_pred_av(rad/s)', 'ai_pred_lv(m/s)'
                                        'dir_high_cmd', 'lat', 'long', 'gps_err',
                                        'mag', 'imu_orient_x',	'imu_orient_y',	'imu_orient_z',	'imu_orient_w',	'imu_ang_vel_x(rad/s)', 'imu_ang_vel_y(rad/s)',
                                        'imu_ang_vel_z(rad/s)',	'imu_lin_acc_x(m/s^2)',	'imu_lin_acc_y(m/s^2)',	'imu_lin_acc_z(m/s^2)',	'sonar_right(m)', 'lidar_front_right(m)',
                                        'sonar_front_center(m)', 'lidar_front_center(m)', 'lidar_front_left(m)', 'sonar_left(m)', 'sonar_rear_center(m)', 'angle_to_path', 'path',
                                        'distance_to_path', 'width_path', 'full_route',
    ...
    for idx, curr_timestamp in enumerate(tqdm(sorted(all_timestamps[all_timestamps >= min_timestamp]), desc="Processing timestamps")):
        ...
        if df_rows['speed']['linear_velocity(m/s)'].values[0] <= 0 and (idx < 50 or (idx + 50) > total_idx):
            continue        
        if df_rows['speed']['linear_velocity(m/s)'].values[0] < -0.03:
            continue
        # If we have wrong speed of the input scenario -> skip this timestamp
        if df_rows['speed']['linear_velocity(m/s)'].values[0] < MIN_LIN_VEL[scenario] and abs(df_rows['speed']['angular_velocity(rad/s)'].values[0]) < 0.05:            
            continue
        ...
        real_image_path = SAVE_DIR + '/camera_front/' + images_path['camera_front']
        routed_map_path = routed_map_save_path #routed map path saved from ai_model_test
        ai_pred_lv,ai_pred_av = model(MODEL_PATH)
        ...
        if data_type == 'umtn':
            ...
            data = {
                ...
                'angular_velocity(rad/s)': df_rows['speed']['angular_velocity(rad/s)'].values[0],
                'linear_velocity(m/s)': df_rows['speed']['linear_velocity(m/s)'].values[0],
                'joystick_angular_velocity(rad/s)': df_rows['joystick_speed']['angular_velocity(rad/s)'].values[0],
                'joystick_linear_velocity(m/s)': df_rows['joystick_speed']['linear_velocity(m/s)'].values[0],
                #'ai_pred_av(rad/s)':df_rows['ai_model']['ai_pred_av(rad/s)'].values[0]
                #'ai_pred_lv(m/s)':df_rows['ai_model']['ai_pred_lv(m/s)'].values[0]
                'dir_high_cmd': df_rows['high_cmd']['dir_high_cmd'].values[0],
                'lat': df_rows['gps']['lat'].values[0],
                'long': df_rows['gps']['long'].values[0],
                'gps_err': df_rows['gps']['gps_err'].values[0],
